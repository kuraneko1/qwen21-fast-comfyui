...
| 参照画像2枚で編集 | 25.9 秒 |
| 参照画像3枚で編集 | 36.4 秒 |
| 参照画像1枚で編集（デモの条件: 参照1672x941 → 出力1376x768・12ステップ） | 約20秒（11回の中央値 19.8秒、範囲 17.6〜22.9秒） |

- 1ステップあたり: **1MPで約0.34秒 / 4MPで約4.3秒**（固定費を除いた、ステップ数に比例する部分）。
- 固定費（プロンプトの符号化・VAEデコード・モデルの入れ替え）が**約5.5秒**。
  1024x1024・12ステップの9.6秒のうち、およそ5.5秒がこれで、残りが12ステップ分。
  だからステップ数を12→25に増やしても 9.6 → 14.0 秒（+4.4秒）しか増えない。
  （9.6秒と14.0秒の2点から逆算すると (14.0−9.6)÷(25−12)=0.338秒/step、固定費 5.54秒。
  下の3-2の表は他の作業と同時に測ったため、負荷のぶん約6秒になっている）
...
- **`reference_fit` の挙動**: `match output`（既定）は参照画像を出力解像度に合わせてリサイズする
  （大きい参照は縮小される＝軽くなる）。`keep original size` は参照を元のサイズのまま使う
  （テキストエンコーダに `resolution=0` を渡す）ので、**参照が出力より小さいときだけ軽くなり**、
  大きいときは逆に重くなる。1024x1024の参照では両者ほぼ同じ（実測 15.9秒 vs 16.7秒。
  別プロンプト・別seedなので厳密なA/Bではない）。
- **編集モードの出力サイズは参照画像の縦横比で決まり、`megapixels` が面積を決める**。テキスト
  エンコーダは `resolution` を**面積の平方根**として扱う（参照を `sqrt(resolution²×ratio)` ×
  `sqrt(resolution²÷ratio)` に縮尺する）ので、ノードは `resolution = sqrt(megapixels×1024²)` を渡す。
  実測: 16:9の参照（768x432）＋ `megapixels=2` → 出力 **1920x1088**（面積は1440x1440とほぼ同じ＝
  指定どおり）。`megapixels=0.5` では **992x544**（736x736とほぼ同じ面積）。
  ノードの `info` は潜在表現から読んだ実サイズを報告する（以前はウィジェット値を報告していた）。

### 2-1. 面積の渡し方を直す前後（2026-09-24 の修正）

以前は `resolution` に**ウィジェットの長辺**を渡していたため、`aspect_ratio` が 1:1 以外だと面積が
`max(ratio, 1/ratio)` 倍になっていた（16:9で1.76倍）。`sqrt(面積)` を渡すよう直したので、どの
`aspect_ratio` でも面積は指定どおりになる。1:1（既定）では渡す値が変わらないため、同梱ワークフローと
上の表の数字はそのまま。

| `aspect_ratio` | `megapixels` | 修正前の実際の出力 | 修正後の実際の出力 | 所要（exec） |
|---|---|---|---:|---:|
| 16:9 | 2 | 2560x1440（3.52MP） | **1920x1088（1.99MP）** | 141.9 秒 → 55.9 秒 |
| 16:9 | 4 | 3616x2048（7.06MP・計算値。実機では未実行） | **2720x1536（3.98MP）** | — → 167.5 秒（OOMなし） |
| 1:1 | 1 | 1376x768（1.01MP） | 1376x768（1.01MP・従来どおり） | 18.0 秒 |

条件: 参照 `docs/demo/source.png`（1672x941）・同一プロンプト・seed 987654321 固定・12ステップ・
`reference_fit` = `match output`・RTX 4070 12GB。修正前の値はノードを一時的に旧版へ戻して実測。
...
| エンジン | 重み | 時間 | 1ステップ | 備考 |
|---|---|---:|---:|---|
| **ComfyUI（このリポジトリの手順）** | 公式 `int8_convrot` | **11.8 秒** | 0.52 秒 | 12GBに常駐（重みの常駐分が約9.5GB。nvidia-smi のピークは11.6〜11.7GB）。W8A8 cuBLAS |
...
- したがって Unsloth Desktop の 2.15 秒/step は「4bit重みを毎ステップbf16へ展開する」コストと、
...
2. **GGUF の再パックは ComfyUI では遅い**。unsloth の **DiT GGUF は `kv_count=0`**（メタデータ無し）で、
   ...
   なる。テキストエンコーダの GGUF は**正しいメタデータ**（`general.architecture=qwen3vl`、kv 42件、
